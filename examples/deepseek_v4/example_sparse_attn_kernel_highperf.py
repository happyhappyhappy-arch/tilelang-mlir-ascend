        ...
        with T.Kernel(num_kernels, is_npu=True) as (kernel_id, subid):
            ...
            with T.Scope("Cube"):
                ...
                for task_id in T.serial(T.ceildiv(num_logic_kernels, num_kernels)):
                    ...
                    if logic_kernel_id < num_logic_kernels:
                        ...
                        T.npuir_load_nd2nz(
                            Q[batch_id, seq_id, 0, 0], l1_q, size=[heads, full_dim]
                        )
                        ...
                        for block_i_id in T.serial(T.ceildiv(top_k, block_I)):
                            ...
                            with T.rs("PIPE_FIX"):
                                T.npuir_store_fixpipe(
                                    l0_c,
                                    workspace_s[kernel_id, block_i_id, 0, 0],
                                    size=[heads, block_I],
                                    enable_nz2nd=True,
                                )
                        ...
                        for block_i_id in T.serial(T.ceildiv(top_k, block_I)):
                            ...
                            with T.rs("PIPE_MTE2"):
                                T.sync_block_wait(block_i_id)
                                T.npuir_load_nd2nz(
                                    workspace_p[kernel_id, block_i_id, 0, 0],
                                    l1_p,
                                    size=[heads, block_I],
                                )
                            ...
                            T.npuir_store_fixpipe(
                                l0_c,
                                workspace_o[kernel_id, block_i_id, 0, 0],
                                size=[heads, dim],
                                enable_nz2nd=True,
                            )
            ...
            with T.Scope("Vector"):
                ...
                ub_cross_kernel_32 = T.alloc_ub(
                    [heads_half, share_block_IK], accum_dtype
                )
                ...
                for task_id in T.serial(T.ceildiv(num_logic_kernels, num_kernels)):
                    ...
                    if logic_kernel_id < num_logic_kernels:
                        ...
                        for block_i_id in T.serial(T.ceildiv(top_k, block_I)):
                            ...
                            tail_size_i_half = (
                                tail_size_i_half - (tail_size_i % 2) * subid
                            )

                            # Load indices and gather sparse KV data (only for first head block)
                            T.copy(
                                Indices[batch_id, seq_id, block_i_offset],
                                ub_indices,
                                size=[1, block_I],
                            )
                            if tail_size_i_half > 0:
                                ...
                                for idx_id in T.serial(tail_size_i_half):
                                    current_index = ub_indices[
                                        0, block_i_sub_offset + idx_id
                                    ]
                                    if (
                                        current_index >= 0
                                        and current_index < seq_len_kv
                                    ):
                                        T.copy(
                                ...
                                T.copy(
                                    ...
                                    workspace_kv[
                                        kernel_id,
                                        block_i_offset + block_i_sub_offset,
                                        0,
                                    ],
                        ...
                        for block_i_id in T.serial(T.ceildiv(top_k, block_I)):
                            ...
                            with T.rs("PIPE_MTE2"):
                                ...
                                T.copy(
                                    workspace_s[
                                        kernel_id, block_i_id, subid * heads_half, 0
                                    ],
                                    ub_cross_kernel_32,
                            ...
                            T.npuir_mul(
                                ub_cross_kernel_32, acc_s_scale, ub_cross_kernel_32
                            )
                            T.npuir_reduce(
                                ub_cross_kernel_32,
                                ub_var_scores_max,
                            ...
                            T.npuir_mul(
                                ub_var_logsum, ub_var_scores_scale, ub_var_logsum
                            )
                        ...
                        T.copy(
                            attn_sink[subid * heads_half : (subid + 1) * heads_half],
                            ub_var_scores_max_prev[:, 0],
                        )
                        T.npuir_sub(
                            ub_var_scores_max_prev,
                            ub_var_scores_max,
                        ...
                        T.npuir_add(
                            ub_var_logsum, ub_var_scores_max_prev, ub_var_logsum
                        )
                        ...
                        for block_k_id in T.serial(T.ceildiv(dim, block_K)):
                            ...
                            T.copy(
                                ub_cross_kernel_16,
                                Output[
                                    batch_id, seq_id, subid * heads_half, block_k_offset
                                ],
                                size=[heads_half, tail_size_k],
                            )
...
def sparse_attn(
    ...
    w_s = torch.empty(
        (num_kernels, num_block_i, heads, block_I), dtype=attn_sink.dtype
    ).npu()
    w_p = torch.empty((num_kernels, num_block_i, heads, block_I), dtype=q.dtype).npu()
    w_o = torch.empty(
        (num_kernels, num_block_i, heads, dim), dtype=attn_sink.dtype
    ).npu()
