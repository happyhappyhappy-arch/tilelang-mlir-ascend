    ...
    def __init__(
        ...
        if dtype == torch.float8_e4m3fn:
            self.weight = nn.Parameter(
                torch.rand(out_features, in_features, dtype=dtype)
            )
...
class Compressor(nn.Module):
    ...
    def __init__(
        ...
        self.ape = nn.Parameter(
            torch.rand(compress_ratio, coff * self.head_dim, dtype=torch.float32)
        )
...
class Attention(nn.Module):
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        self.attn_sink = nn.Parameter(
            torch.rand(self.n_local_heads, dtype=torch.float32)
        )
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        if self.hash:
            ...
            self.bias = nn.Parameter(
                torch.rand(args.n_routed_experts, dtype=torch.float32)
            )
...
class ParallelHead(nn.Module):
    def __init__(
        ...
        self.weight = nn.Parameter(
            torch.rand(self.part_vocab_size, self.dim, dtype=torch.float32)
        )
