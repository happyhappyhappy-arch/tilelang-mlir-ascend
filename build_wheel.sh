...
# --- Stage the unified compiler and bitcode ---
export BISHENGIR_PATH="${BISHENGIR_PATH:-$(pwd)/3rdparty/AscendNPU-IR/build/install}"
mkdir -p 3rdparty/bin 3rdparty/lib
cp -L --preserve=mode "$BISHENGIR_PATH/bin/bishengir-compile" "$BISHENGIR_PATH/bin/hivmc-a5" 3rdparty/bin/
for name in host.bc meta_op.{aic,aiv,mix.aic,mix.aiv}.{c220,c310}.bc; do
